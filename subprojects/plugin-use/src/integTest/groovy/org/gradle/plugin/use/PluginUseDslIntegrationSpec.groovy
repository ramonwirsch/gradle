...
class PluginUseDslIntegrationSpec extends AbstractIntegrationSpec {
    ...
    def "illegal syntax in plugins block - #code"() {
        ...
        2          | "id()"                                 | NEED_SINGLE_ARGUMENT
        2          | "id(1)"                                | NEED_SINGLE_ARGUMENT
        2          | "id(System.getProperty('foo'))"        | NEED_SINGLE_ARGUMENT
        2          | "id('a' + 'b')"                        | NEED_SINGLE_ARGUMENT
        2          | "id(\"\${'foo'}\")"                    | NEED_SINGLE_ARGUMENT
        2          | "version('foo')"                       | BASE_MESSAGE
        2          | "id('foo').version(1)"                 | NEED_SINGLE_ARGUMENT
        2          | "id 'foo' version 1"                   | NEED_SINGLE_ARGUMENT
        ...
        2          | "id('foo').\"\${'version'}\" 'bar'"    | BASE_MESSAGE
        2          | "noApply()"                            | BASE_MESSAGE
        2          | "id 'foo' noApply"                     | BASE_MESSAGE
        2          | "id 'foo' noApply(true)"               | NEED_EMPTY_ARGUMENT
        2          | "id ' '"                               | invalidPluginIdCharMessage(' ' as char)
        2          | "id '\$'"                              | invalidPluginIdCharMessage('$' as char)
        2          | "id ''"                                | NEED_SINGLE_ARGUMENT
        2          | "id 'foo' version ''"                  | NEED_SINGLE_ARGUMENT
        2          | "id null"                              | NEED_SINGLE_ARGUMENT
        2          | "id 'foo' version null"                | NEED_SINGLE_ARGUMENT
    ...
    def "allowed syntax in plugins block - #code"() {
        ...
        code << [
                ...
                "id 'noop' noApply()",
                "id('noop').noApply()",
                "id('noop').noApply();id('java')",
                "id 'noop' version 'bar' noApply()",
                "id('noop').version('bar').noApply()",
        ]
    }
}
